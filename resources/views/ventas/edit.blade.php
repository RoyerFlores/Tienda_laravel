        ...
        <div class="container py-4">
            ...
            <div class="card shadow border-0">
                <div class="card-body p-4">
                    <form action="{{ route('ventas.update', $venta) }}" method="POST" class="row g-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-12 col-lg-6">
                            ...
                            <select name="cliente_id"
                                class="form-select shadow-sm @error('cliente_id') is-invalid @enderror" required>
                        ...
                        <div class="col-12 col-lg-6">
                            ...
                            <select name="producto_id" id="producto_id"
                                class="form-select shadow-sm @error('producto_id') is-invalid @enderror" required>
                                <option value="">Seleccione un producto</option>
                                @foreach($productos as $producto)
                                    <option value="{{ $producto->id }}" 
                                            data-precio="{{ $producto->precio_venta }}"
                                            data-stock="{{ $producto->stock + ($venta->producto_id == $producto->id ? $venta->cantidad : 0) }}"
                                            {{ (old('producto_id', $venta->producto_id) == $producto->id) ? 'selected' : '' }}>
                                        {{ $producto->nombre }} 
                                        (Stock: {{ $producto->stock + ($venta->producto_id == $producto->id ? $venta->cantidad : 0) }}) 
